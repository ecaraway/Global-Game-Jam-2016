...
public class CollisionAvoidance : MonoBehaviour {
    ...
    public Vector2 getSteering(ICollection<Rigidbody2D> targets)
    {
        ...
        foreach (Rigidbody2D r in targets)
        {
			...
			Vector2 relativePos = new Vector2(transform.position.x, transform.position.y) - r.position;
        ...
        {
			acceleration = new Vector2(transform.position.x, transform.position.y) - firstTarget.position;
        }
